...
class AxGraph(ig.Graph):
    ...
    def __init__(self, edge_width=1, *args, **kwargs):
        super(AxGraph, self).__init__(*args, **kwargs)
        self.edge_width = edge_width
        self.q = None

    @classmethod
    def random_graph_with_attrs(cls, N=500, av_k=4.0, f=3, q=2, edge_width=1, init_states=None):
        ...
        if init_states is not None:
            g.vs()["f"] = np.int_(init_states * q)
        else:
            g.vs()["f"] = np.int_(rand((N, f)) * q)
        g.edge_width = edge_width
    ...
    def copy(self, q=None, init_states=None, *args, **kwargs):
        res = super(AxGraph, self).copy()
        res.edge_width = self.edge_width
        if q is None:
            res.q = self.q
        else:
            res.q = q
        if init_states is None:
            ...
            res.vs['f'] = np.int_(init_states * res.q)
        res.es['width'] = self.edge_width
        return res

    def add_edges(self, *args, **kwargs):
        res = super(AxGraph, self).add_edges(*args, **kwargs)
        self.es()['width'] = self.edge_width
        return res

    def get_colors(self, traits):
        R = hex(int(255.0 * traits[0] / self.q)).replace('0x', '').zfill(2)
        G = hex(int(255.0 * traits[1] / self.q)).replace('0x', '').zfill(2)
        B = hex(int(255.0 * traits[2] / self.q)).replace('0x', '').zfill(2)
        return '#{}{}{}'.format(R, G, B)

    def set_all_colors(self):
        colors = []
        for traits in self.vs['f']:
            colors.append(self.get_colors(traits))
        self.vs['color'] = colors

    def set_colors(self, index):
        self.vs(index)['color'] = self.get_colors(self.vs(index)['f'][0])
...
class AxSimulation:
    ...
    def basic_algorithm(self, g, T, n):
        """This is the basic algorithm of axelrod's coevolving network.
        @param g: graph to work on
        @param T: number of time steps
        @return: g after applying algorithm
